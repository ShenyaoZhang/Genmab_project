...
def generate_shap_summary_plot(
        ...
        X_test,
        feature_names,
        output_file='crs_shap_summary_presentation.png'):
    """
    ...
    try:
        ...
        # Handle binary classification (shap_values might be a list or array with
    ...
    plt.title(
        'SHAP Summary Plot: CRS → Death Model\n(Which Factors Matter Most?)',
        fontsize=16,
        fontweight='bold',
        pad=20)
    plt.tight_layout()
...
def load_crs_data_for_stratification():
    ...
    print("Loaded from main_data.csv")
        ...
        print("ERROR: Data file not found: main_data.csv or preprocessed_data.csv")

    return None

    # Filter to Epcoritamab patients
    epcor_df = df[df['target_drug'].str.contains(
        'Epcoritamab', case=False, na=False)].copy()

    if len(epcor_df) == 0:
        print("ERROR: No Epcoritamab patients found")
    ...
    if len(crs_df) == 0:
        print("ERROR: No CRS patients found")

    return None

    # Add death flag
    crs_df['death'] = pd.to_numeric(
        crs_df['seriousnessdeath'],
    ...
    if 'age_years' not in crs_df.columns and 'patientonsetage' in crs_df.columns:
        crs_df['age_years'] = pd.to_numeric(
            crs_df['patientonsetage'], errors='coerce')

    return crs_df


def generate_granular_risk_chart(
        crs_df,
        output_file='crs_granular_risk_stratification.png'):
            """
    ...
    if crs_df is None or len(crs_df) == 0:
        print("ERROR: No CRS data available")
    ...
    crs_df['age_group'] = pd.cut(
        age,
        bins=age_bins,
        labels=age_labels,
        include_lowest=True)

    age_stats = crs_df.groupby('age_group')['death'].agg(
    ...
    age_stats['n_death'] = (
        age_stats['death_rate'] *
        age_stats['n_total']).fillna(0).astype(int)
    stratifications.append(
        age_stats[['category', 'subcategory', 'death_rate', 'n_total', 'n_death']])
    ...
    if 'num_drugs' in crs_df.columns:
        crs_df['polypharmacy_group'] = pd.cut(
            ...
            labels=[
                ...
                'Very High (>10)'],
            include_lowest=True)
    poly_stats = crs_df.groupby('polypharmacy_group')[
        'death'].agg(['mean', 'count']).reset_index()
    ...
    poly_stats['n_death'] = (
        poly_stats['death_rate'] *
        poly_stats['n_total']).fillna(0).astype(int)
    stratifications.append(
        poly_stats[['category', 'subcategory', 'death_rate', 'n_total', 'n_death']])

    # Combine all stratifications
    if not stratifications:
        print("ERROR: No stratification data available")
    ...
    for cat in categories:
        cat_df = combined_df[combined_df['category'] ==
                             cat].sort_values('death_rate', ascending=False)
    ...
    ax.set_xticklabels([label.split('\n')[0]
                       for label in bar_labels], rotation=45, ha='right', fontsize=10)
...
def main():
    ...
    if model is not None and X_test is not None:
        ...
        print("WARNING: Skipping SHAP plot (model/data not available)")
    ...
    if crs_df is not None:
        ...
        print("WARNING: Skipping risk stratification (data not available)")
