...
if len(df_epc) == 0:
    print("❌ Error: no Epcoritamab records found")
    sys.exit(1)
...
death_count = pd.to_numeric(df_epc['seriousnessdeath'], errors='coerce').fillna(0).sum()
hosp_count = pd.to_numeric(df_epc['seriousnesshospitalization'], errors='coerce').fillna(0).sum()
life_count = pd.to_numeric(df_epc['seriousnesslifethreatening'], errors='coerce').fillna(0).sum()

print(f"  Death cases: {int(death_count)} ({death_count/len(df_epc)*100:.1f}%)")
print(f"  Hospitalisation cases: {int(hosp_count)} ({hosp_count/len(df_epc)*100:.1f}%)")
print(f"  Life-threatening cases: {int(life_count)} ({life_count/len(df_epc)*100:.1f}%)")
print()
...
if 'patientsex' in df_epc.columns:
    df_epc['patientsex'] = pd.to_numeric(df_epc['patientsex'], errors='coerce').fillna(0).astype(int)

if 'patientonsetage' in df_epc.columns:
    df_epc['patientonsetage'] = pd.to_numeric(df_epc['patientonsetage'], errors='coerce')
...
if 'patientweight' in df_epc.columns:
    df_epc['patientweight'] = pd.to_numeric(df_epc['patientweight'], errors='coerce')

if 'num_drugs' in df_epc.columns:
    df_epc['num_drugs'] = pd.to_numeric(df_epc['num_drugs'], errors='coerce').fillna(1).astype(int)

if 'num_reactions' in df_epc.columns:
    df_epc['num_reactions'] = pd.to_numeric(df_epc['num_reactions'], errors='coerce').fillna(1).astype(int)
...
if 'num_reactions' in df_epc.columns:
    df_epc['multiple_reactions'] = (df_epc['num_reactions'] > 1).astype(int)
...
if not os.path.exists("X_train.csv"):
    print("❌ Error: missing reference feature file X_train.csv")
...
missing_features = [col for col in reference_features if col not in df_epc.columns]
...
if os.path.exists("model_comparison.csv"):
    ...
    for model_name in results.keys():
        if model_name in all_drugs_results.index:
            print(f"{model_name}:")
            print(f"  Accuracy (overall) {all_drugs_results.loc[model_name, 'accuracy']:.4f} | Epcoritamab {results_df.loc[model_name, 'accuracy']:.4f}")
            print(f"  F1 score (overall) {all_drugs_results.loc[model_name, 'f1']:.4f} | Epcoritamab {results_df.loc[model_name, 'f1']:.4f}")
            print()
...
print(f" 2. Mortality rate: {float(positive/len(y_test_epc)*100):.1f}%")
print(f" 3. Best model performance: {results_df.loc[best_model, 'f1']:.4f} (F1 score)")
