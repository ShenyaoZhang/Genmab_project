...
def extract_comorbidities(drug_indication_str, reactions_str=None, all_drugs_str=None):
    """
    ...
    if reactions_str is not None and not pd.isna(reactions_str) and reactions_str != '':
        all_text.append(str(reactions_str).upper())
    
    if all_drugs_str is not None and not pd.isna(all_drugs_str) and all_drugs_str != '':
...
def granular_feature_engineering(df):
    ...
    if 'patientweight' in feature_df.columns:
        ...
        print(f"   Obese (BMI>30): {n_obese} patients ({n_obese/n_with_bmi*100:.1f}%)" if n_with_bmi > 0 else "   No BMI data")
    ...
    if 'drug_indication' in feature_df.columns:
        ...
        for comorb in COMORBIDITY_KEYWORDS.keys():
            ...
            print(f"   {comorb.capitalize()}: {n_comorb} patients ({n_comorb/len(feature_df)*100:.1f}%)")
    ...
    if 'all_drugs' in feature_df.columns:
        ...
        print(f"   Steroid + Antibiotic: {feature_df['steroid_plus_antibiotic'].sum()} patients")
        print(f"   Steroid + Antifungal: {feature_df['steroid_plus_antifungal'].sum()} patients")
    ...
    if 'reactions' in feature_df.columns:
        ...
        for keyword in infection_ae_keywords:
            feature_df['has_infection_ae'] |= reactions_upper.str.contains(keyword, na=False)
        feature_df['has_infection_ae'] = feature_df['has_infection_ae'].astype(int)
        n_infection_ae = feature_df['has_infection_ae'].sum()
        print(f"   Infection-related AE: {n_infection_ae} patients ({n_infection_ae/len(feature_df)*100:.1f}%)")
    ...
    if 'drug_indication' in feature_df.columns:
        ...
        print(f"   Patients with stage information: {total_with_stage} ({total_with_stage/len(feature_df)*100:.1f}%)")
...
def stratified_analysis(df):
    ...
    if 'age_years' in crs_df.columns:
        ...
        if age_gt_65.sum() > 0:
            ...
            print(f"   Age >65: {n_death_gt_65}/{n_gt_65} deaths ({death_rate_gt_65*100:.1f}%)")
            findings.append({
                'variable': 'Age',
                'category': '>65 years',
        ...
        if age_le_65.sum() > 0:
            ...
            print(f"   Age ≤65: {n_death_le_65}/{n_le_65} deaths ({death_rate_le_65*100:.1f}%)")
            findings.append({
                'variable': 'Age',
                'category': '≤65 years',
    ...
    if 'bmi_obese' in crs_df.columns:
        ...
        if len(obese_crs) > 0:
            ...
            print(f"   Obese (BMI>30): {obese_crs['death'].sum()}/{len(obese_crs)} deaths ({death_rate_obese*100:.1f}%)")
            findings.append({
                'variable': 'BMI',
        ...
        if len(non_obese_crs) > 5:  # Only if enough samples
            ...
            print(f"   Non-obese: {non_obese_crs['death'].sum()}/{len(non_obese_crs)} deaths ({death_rate_non_obese*100:.1f}%)")
            findings.append({
                'variable': 'BMI',
                'category': 'Non-obese',
    ...
    for comorb in COMORBIDITY_KEYWORDS.keys():
        ...
        if comorb_col in crs_df.columns:
            ...
            if len(with_comorb) > 0:
                ...
                print(f"   {comorb.capitalize()}: {with_comorb['death'].sum()}/{len(with_comorb)} deaths ({death_rate*100:.1f}%)")
                findings.append({
    ...
    if 'steroid_plus_antibiotic' in crs_df.columns:
        ...
        if len(combo) > 0:
            ...
            print(f"   Steroid + Antibiotic: {combo['death'].sum()}/{len(combo)} deaths ({death_rate*100:.1f}%)")
            findings.append({
    ...
    if 'has_infection_ae' in crs_df.columns and 'comorbidity_diabetes' in crs_df.columns:
        infection_diabetes = crs_df[(crs_df['has_infection_ae'] == 1) & (crs_df['comorbidity_diabetes'] == 1)]
        if len(infection_diabetes) > 0:
            death_rate = infection_diabetes['death'].mean()
            print(f"   Infection AE + Diabetes: {infection_diabetes['death'].sum()}/{len(infection_diabetes)} deaths ({death_rate*100:.1f}%)")
            findings.append({
    ...
    # Check if stage features exist (from 03_preprocess_data.py or 12_crs_model_training.py)
    stage_features = ['cancer_stage_I', 'cancer_stage_II', 'cancer_stage_III', 'cancer_stage_IV']
    ...
    if has_stage_features:
        ...
        if early_stage_mask.sum() > 0:
            ...
            print(f"   Stage I-II (Early): {n_death_early}/{n_early} deaths ({death_rate_early*100:.1f}%)")
            findings.append({
        ...
        if advanced_stage_mask.sum() > 0:
            ...
            print(f"   Stage III-IV (Advanced): {n_death_advanced}/{n_advanced} deaths ({death_rate_advanced*100:.1f}%)")
            findings.append({
                'variable': 'Cancer Stage',
        ...
        print("   ⚠️  Cancer stage features not found in dataset")
...
def generate_granular_report(findings, combo_analysis, output_file='granular_crs_report.md'):
    ...
    if age_findings:
        ...
        for finding in age_findings:
            report.append(f"- **Age {finding['category']} CRS patients:** {finding['death_rate']*100:.1f}% death rate ({finding['n_deaths']}/{finding['n_total']} patients)")
        report.append("\n**Interpretation:** " + 
    ...
    if bmi_findings:
        ...
        for finding in bmi_findings:
            report.append(f"- **{finding['category']} CRS patients:** {finding['death_rate']*100:.1f}% death rate ({finding['n_deaths']}/{finding['n_total']} patients)")
        report.append("\n**Interpretation:** " +
    ...
    if comorb_findings:
        ...
        for finding in comorb_findings:
            report.append(f"- **CRS patients with {finding['category']}:** {finding['death_rate']*100:.1f}% death rate ({finding['n_deaths']}/{finding['n_total']} patients)")
        report.append("\n")
    ...
    if combo_findings:
        ...
        for finding in combo_findings:
            report.append(f"- **CRS patients on {finding['category']}:** {finding['death_rate']*100:.1f}% death rate ({finding['n_deaths']}/{finding['n_total']} patients)")
        report.append("\n")
    ...
    if combo_analysis:
        ...
        for i, combo in enumerate(combo_analysis[:10], 1):
            ...
            report.append(f"| {i} | {drug1_short} | {drug2_short} | {combo['death_rate']*100:.1f}% | {combo['n_death']}/{combo['n_total']} |\n")
        report.append("\n")
    ...
    if age_findings and len(age_findings) >= 2:
        ...
        if gt_65 and le_65:
            report.append(f"**Finding 1:** We found that CRS patients aged >65 years had a {gt_65['death_rate']*100:.1f}% death rate, ")
            report.append(f"compared to {le_65['death_rate']*100:.1f}% in patients aged ≤65 years.\n\n")
    
    if combo_findings:
        for finding in combo_findings:
            report.append(f"**Finding 2:** CRS patients using {finding['category']} had a {finding['death_rate']*100:.1f}% death rate ")
    ...
    if infection_findings:
        ...
        report.append(f"{infection_findings[0]['death_rate']*100:.1f}% death rate ({infection_findings[0]['n_deaths']}/{infection_findings[0]['n_total']} patients).\n\n")
