...
def load_crs_data():
    ...
    if not os.path.exists('main_data.csv'):
        print("ERROR: Data file not found: main_data.csv")
...
def generate_global_summary(top_positive, top_negative, feature_names_map):
    ...
    age_features = [
        f for f in top_features_list if 'age' in f.lower() or 'Age' in f]
    polypharmacy_features = [
        f for f in top_features_list if 'polypharmacy' in f.lower() or 'Medications' in f]
    comorbidity_features = [
        f for f in top_features_list if 'History' in f or 'Diabetes' in f or 'Hypertension' in f or 'Cardiac' in f]
    infection_features = [f for f in top_features_list if 'Infection' in f]
    drug_combo_features = [
        f for f in top_features_list if 'Combination' in f or 'Steroid' in f]
    ...
    if main_factors:
        factors_str = ", ".join(main_factors[:-1]) + \

        (f", and {main_factors[-1]}" if len(main_factors)
         > 1 else main_factors[0])
...
def generate_local_explanations(
    ...
    feature_contributions = list(
        zip(display_names, patient_shap, patient_data))
    ...
    patient_desc = " and ".join(
        patient_desc_parts) if patient_desc_parts else "complex clinical presentation"
...
def generate_shap_explanations(model_data, X_test, y_test, output_dir='.'):
    ...
    if scaler:
        ...
        X_test_scaled = X_test.values if isinstance(
    ...
    explainer = shap.KernelExplainer(
        model.predict_proba, X_test_scaled[:50])  # Sample for speed
    ...
    print(f" SHAP values calculated (shape: {shap_values.shape})")
        ...
        print(f"WARNING: TreeExplainer failed, trying KernelExplainer: {e}")
    ...
    shap.summary_plot(
        ...
        show=False)
    plt.title(
        'SHAP Summary Plot: CRS → Death Prediction\n(Higher values push toward death)',
        fontsize=14,
        fontweight='bold')
    plt.tight_layout()
    plt.savefig(
        f'{output_dir}/crs_shap_summary.png',
        dpi=300,
        bbox_inches='tight')
    ...
    if isinstance(explainer.expected_value, np.ndarray):
        base_value = explainer.expected_value[1] if len(
            explainer.expected_value) > 1 else explainer.expected_value[0]
    ...
    plt.tight_layout()
    plt.savefig(
        f'{output_dir}/crs_shap_waterfall_death.png',
        dpi=300,
        bbox_inches='tight')
    ...
    plt.close()
        ...
        print(
            f"WARNING: Could not generate waterfall plot for death case: {e}")
    ...
    plt.tight_layout()
    plt.savefig(
        f'{output_dir}/crs_shap_waterfall_survival.png',
        dpi=300,
        bbox_inches='tight')
    ...
    plt.close()
        ...
        print(
            f"WARNING: Could not generate waterfall plot for survival case: {e}")
...
def generate_plain_language_summary(
        shap_results=None,
        feature_importance_file='crs_feature_importance.csv'):
    ...
    desc = feature_descriptions.get(
        feat, f"This factor contributes to death risk.")

    summary.append(f"{i + 1}. **{feat.replace('_',
                                              ' ').title()}** (Importance: {imp:.3f})\n")
...
def main():
    ...
    if HAS_SHAP:
        ...
        print("WARNING: Test data not found in model file.")
