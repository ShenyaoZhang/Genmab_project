...
missing_df = missing_df[missing_df['Missing'] >
                        0].sort_values('Missing', ascending=False)
...
if 'receivedate' in df.columns:
    ...
    try:
        df['receivedate_parsed'] = pd.to_datetime(
            df['receivedate'], format='%Y%m%d', errors='coerce')
        ...
        print(
            f"Valid dates: len(valid_dates)} / {len(df){len(valid_dates)}/
                len(df) *
                100:.1f}%)")
    ...
    # Distribution by year
        ...
        print(f"WARNING: Date parsing failed: {str(e)[:50]}")
...
# Severity consistency check
    ...
    print(
        f" Inconsistent records: {inconsistent}{ inconsistent /
            len(df) *
            100:.2f}%)")
    if inconsistent > 0:
...
if 'num_drugs' in df.columns:
    ...
    print(
        f" Polypharmacy (>1 drug): {polypharmacy}{ polypharmacy /
            len(df) *
            100:.1f}%)")
    print()
...
# Identify Epcoritamab records
    if 'target_drug' in df.columns:
        epcor_mask = df['target_drug'].str.contains(
        'Epcoritamab', case=False, na=False)
    ...
    if 'seriousnessdeath' in epcor_df.columns:
        ...
        if len(crs_death_df) > 0:
            crs_death_col = pd.to_numeric(
        crs_death_df['seriousnessdeath'],
        errors='coerce').fillna(0)
    n_crs_death = (crs_death_col > 0).sum()
    crs_death_rate = (n_crs_death / len(crs_death_df)) * \
        100 if len(crs_death_df) > 0 else 0

    print(f"WARNING: CRS → Death:")
    print(
        f" Deaths in CRS patients: {n_crs_death} / {len(crs_death_df)}{crs_death_rate:.1f}%)")
    print()
    ...
    print(
        f"Among these, {n_crs} reports ( crs_rate:.1f}%) contain CRS-related adverse events.")
    if 'seriousnessdeath' in epcor_df.columns and len(crs_death_df) > 0:
        ...
        if 'seriousnessdeath' in epcor_df.columns:
            print(
        f" • Total deaths in Epcoritamab reports: {n_death}{ death_rate:.1f}%)")
    if len(crs_death_df) > 0:
        print(
            f" • Deaths in CRS patients: {n_crs_death} / {n_crs}{crs_death_rate:.1f}%)")
        print()
        else:
...
if len(df) >= 1000:
    ...
    print(" WARNING: Data volume is small but still usable for preliminary analysis")
print()
