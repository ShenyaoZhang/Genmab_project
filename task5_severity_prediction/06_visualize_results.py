...
def get_best_model_name_by_prauc(model_df: pd.DataFrame) -> str:
    ...
    if "pr_auc" not in model_df.columns:
        raise ValueError(
            "model_comparison.csv is missing 'pr_auc'; cannot choose best model.")
...
# -----------------------------
    ...
    if 'seriousnessdeath' in df.columns:
        ...
        axes[1, 1].bar(
            [0, 1], [death_counts.get(0, 0), death_counts.get(1, 0)])
...
# -----------------------------
        ...
        best_scores = get_model_scores_series(
            model_results, best_model_by_prauc)
...
model_files = [f for f in os.listdir('.') if f.startswith(
    'trained_model_') and f.endswith('.pkl')]
...
try:
    ...
    if not os.path.exists(best_model_file):
        print(
            f"WARNING: Best model file not found: {best_model_file}，skipping advanced curves generation")
                elif not os.path.exists("X_test.csv") or not os.path.exists("y_test.csv"):
                    print("WARNING: X_test/y_test missing; skipping advanced curves")
    ...
    # Probability or confidence score
        ...
        if y_pred_proba is None:
            print("WARNING: Model lacks probability/decision outputs; skipping curves")
    ...
    if len(thresholds) > 0:
        ...
        axes[0].scatter([recall[best_idx]],
                        [precision[best_idx]],
                        color='red',
                        s=100,
                        zorder=5,
    ...
    print(
        f" PR-AUC={pr_auc:.3f}, ROC-AUC={roc_auc:.3f}, F1Best threshold={best_threshold:.3f}\n")
    ...
    print(" For advanced curves ensure: pip install scikit-learn matplotlib seaborn")
        ...
        print(f"WARNING: Error generating advanced curves: {str(e)[:100]}")
