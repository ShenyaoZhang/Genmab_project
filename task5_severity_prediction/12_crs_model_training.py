...
def identify_crs_patients(df, drug_name=None, ae_keywords=None):
    ...
    if len(drug_df) == 0:
        print(f"ERROR: No {drug_name} records found.")
        return None
    ...
    if len(ae_df) == 0:
        print(f"ERROR: No patients with {ae_name} found.")
        return None
    # Outcome: death
    ae_df['death'] = pd.to_numeric(
        ae_df['seriousnessdeath'],
        errors='coerce').fillna(0).astype(int)
    n_death = ae_df['death'].sum()
    print(
        f" Deaths in {ae_name} patients: {n_death}{ n_death /
            len(ae_df) *
            100:.1f}%)")
    return ae_df
def build_crs_features(df):
    ...
    if 'age_years' in feature_df.columns:
        feature_df['age_years'] = pd.to_numeric(
            feature_df['age_years'], errors='coerce')
        feature_df['age_missing'] = feature_df['age_years'].isna().astype(int)
        feature_df['age_years'] = feature_df['age_years'].fillna(
            feature_df['age_years'].median())
    ...
    feature_df['age_50_65'] = (
        (feature_df['age_years'] >= 50) & (
            feature_df['age_years'] <= 65)).astype(int)
    ...
    if 'patientsex' in feature_df.columns:
        feature_df['sex_male'] = (
            pd.to_numeric(
                feature_df['patientsex'],
                errors='coerce') == 1).astype(int)
        feature_df['sex_female'] = (
            pd.to_numeric(
                feature_df['patientsex'],
                errors='coerce') == 2).astype(int)
        feature_df['sex_unknown'] = (
            pd.to_numeric(
                feature_df['patientsex'],
                errors='coerce').isna()).astype(int)
    ...
    # Add all BMI-related features to the model (including underweight for
    # anorexic-like patients)
    ...
    if 'num_drugs' in feature_df.columns:
        feature_df['num_drugs'] = pd.to_numeric(
            feature_df['num_drugs'],
            errors='coerce').fillna(0).astype(int)
        feature_df['polypharmacy'] = (feature_df['num_drugs'] > 1).astype(int)
        feature_df['high_polypharmacy'] = (
            feature_df['num_drugs'] > 5).astype(int)
    ...
    if 'num_reactions' in feature_df.columns:
        feature_df['num_reactions'] = pd.to_numeric(
            feature_df['num_reactions'],
            errors='coerce').fillna(0).astype(int)
        feature_df['multiple_reactions'] = (
            feature_df['num_reactions'] > 1).astype(int)
    ...
    if 'all_drugs' in feature_df.columns:
        ...
        feature_df['has_antibiotic'] = (
            drugs_upper.str.contains(
                'CIPROFLOXACIN|LEVOFLOXACIN|VANCOMYCIN|AMOXICILLIN',
                na=False)).astype(int)
        ...
        feature_df['has_targeted'] = (
            drugs_upper.str.contains(
                'RITUXIMAB|BRENTUXIMAB|LENALIDOMIDE',
                na=False)).astype(int)
        features.extend([
    ...
    if 'drug_indication' in feature_df.columns:
        ...
        feature_df['comorbidity_cardiac'] = (
            indication_upper.str.contains(
                'CARDIAC|HEART|CARDIOMYOPATHY',
                na=False)).astype(int)
        features.extend(['comorbidity_diabetes',
    ...
    if 'cancer_stage' in feature_df.columns:
        ...
        stage_numeric = pd.to_numeric(
            feature_df['cancer_stage'], errors='coerce')
        if not stage_numeric.isna().all():
            feature_df['cancer_stage_numeric'] = stage_numeric.fillna(0)
            feature_df['advanced_stage'] = (
                (stage_numeric >= 3) & (
                    stage_numeric <= 4)).astype(int)
    ...
    if 'drug_indication' in feature_df.columns:
        ...
        # Match patterns like: "STAGE I", "STAGE 1", "STAGE II", "STAGE 2",
        # "STAGE III", "STAGE 3", "STAGE IV", "STAGE 4"
        ...
        feature_df['cancer_stage_II'] = (
            feature_df['cancer_stage_II'] | indication_upper.str.contains(
                r'STAGE\s+2(\s|$|,|\|)', regex=True, na=False)).astype(int)
        # Stage III/3
        ...
        feature_df['cancer_stage_IV'] = (
            feature_df['cancer_stage_IV'] | indication_upper.str.contains(
                r'STAGE\s+4(\s|$|,|\|)', regex=True, na=False)).astype(int)
        ...
        features.extend(['cancer_stage_I', 'cancer_stage_II',
                        'cancer_stage_III', 'cancer_stage_IV'])
    # Print statistics
    ...
    if 'reactions' in feature_df.columns:
        ...
        feature_df['has_infection_ae'] = (
            reactions_upper.str.contains(
                'INFECTION|SEPSIS|PNEUMONIA|BACTEREMIA|ASPERGILLOSIS',
                na=False)).astype(int)
    ...
    print(f" Features: {', '.join(available_features[:15])}..." if len(
        available_features) > 15 else f" Features: {', '.join(available_features)}")
...
def train_crs_models(X, y, random_state=42):
    ...
    # Check data size
    if len(X) < 10:
        print(f"ERROR: Insufficient data: only {len(X)} samples.")
    return None
    positive_rate = y.sum() / len(y)
    print(
        f"\n Dataset: len(X)} CRS patients, {y.sum()}deaths ({
                positive_rate *
            100:.1f}% positive)")
    if positive_rate == 0 or positive_rate == 1:
        print("WARNING: Cannot train: all outcomes are the same")
    return None
    # Train-test split
    if len(X) >= 30:
    ...
    if len(X_train) >= 20:
        ...
        print("WARNING: Small dataset: using all data for training (no test split)")
    X_tr, X_val, y_test = X, X, X
    y_tr, y_val, y_test = y, y, y
    print(
        f" Train: len(X_tr)}, Validation: {len(X_val)}, Test: {len(X_test)}")
    ...
    X_test_scaled = scaler.transform(X_test) if len(
        X_test) < len(X) else scaler.transform(X_test)
    models = {}
    results = {}
    feature_importances = {}
    ...
    results['Logistic Regression'] = {
        'roc_auc': roc_auc_score(
            y_test, lr_proba_test) if len(
            np.unique(y_test)) > 1 else 0.5, 'pr_auc': average_precision_score(
                y_test, lr_proba_test), 'f1': f1_score(
                    y_test, lr_pred), 'accuracy': accuracy_score(
                        y_test, lr_pred), 'precision': precision_score(
                            y_test, lr_pred, zero_division=0), 'recall': recall_score(
    ...
    results['Random Forest'] = {
        'roc_auc': roc_auc_score(
            y_test, rf_proba_test) if len(
            np.unique(y_test)) > 1 else 0.5, 'pr_auc': average_precision_score(
                y_test, rf_proba_test), 'f1': f1_score(
                    y_test, rf_pred), 'accuracy': accuracy_score(
                        y_test, rf_pred), 'precision': precision_score(
                            y_test, rf_pred, zero_division=0), 'recall': recall_score(
    ...
    results['Gradient Boosting'] = {
        'roc_auc': roc_auc_score(
            y_test, gb_proba_test) if len(
            np.unique(y_test)) > 1 else 0.5, 'pr_auc': average_precision_score(
                y_test, gb_proba_test), 'f1': f1_score(
                    y_test, gb_pred), 'accuracy': accuracy_score(
                        y_test, gb_pred), 'precision': precision_score(
                            y_test, gb_pred, zero_division=0), 'recall': recall_score(
    ...
    results['XGBoost'] = {
        'roc_auc': roc_auc_score(
            y_test, xgb_proba_test) if len(
            np.unique(y_test)) > 1 else 0.5, 'pr_auc': average_precision_score(
                y_test, xgb_proba_test), 'f1': f1_score(
                    y_test, xgb_pred), 'accuracy': accuracy_score(
                        y_test, xgb_pred), 'precision': precision_score(
                            y_test, xgb_pred, zero_division=0), 'recall': recall_score(
    ...
    print(
        f" Positive (Death): {y_test.sum()}/{len(y_test)}{test_positive_rate * 100:.1f}%)")
    print(
        f" Negative (Survival): {(y_test == 0).sum()}/{len(y_test)}{test_negative_rate * 100:.1f}%)")
    print(
        f" Imbalance Ratio: test_negative_rate /
            test_positive_rate:.2f}:1" if test_positive_rate > 0 else " Imbalance Ratio: N/A")
    print("\n" + "-" * 80)
    ...
    # Note: thresholds_val has length len(recall_val) - 1, so we need to
    # handle indexing carefully
    target_recall = 0.70
    # Exclude last element (recall=1.0, no threshold)
    recall_idx = np.argmin(np.abs(recall_val[:-1] - target_recall))
    ...
    print("=" * 80)
    print(
        f" Although positive cases represent only test_positive_rate:.1%} of CRS patients, ")
    print(
        f" the model achieves precision_70recall:.1%} precision at {actual_recall_70:.1%}recall, ")
    print(f" indicating potential utility as a screening tool for identifying high-risk CRS patients.")
    print(
        f" The PR-AUC of {best_result['pr_auc']:.3f} demonstrates strong performance on this ")
    print(
        f" imbalanced classification task ( test_negative_rate /
            test_positive_rate:.1f}:1 imbalance ratio).")
    print("=" * 80)
    return {
        'models': models,
        'results': results,
...
def analyze_feature_importance(model_results, output_dir='.'):
    ...
    cancer_stage_features = [
        f for f in feature_cols if 'cancer_stage' in f.lower()]
    ...
    ax.set_title(
        f'Top {top_n} Features for CRS → Death Prediction\n({best_model_name})',
        fontsize=14,
        fontweight='bold')
    ax.invert_yaxis()
    plt.tight_layout()
    ...
    importance_df_complete = pd.DataFrame(
        sorted_features, columns=[
            'feature', 'importance'])
    importance_df_complete.to_csv(
        'crs_feature_importance_complete.csv', index=False)
    print(
        f" Saved: crs_feature_importance_complete.csv ( len(sorted_features)} features - complete list)")
    # Also save top 15 for quick reference
    importance_df_top15 = pd.DataFrame(
        sorted_features[:15], columns=['feature', 'importance'])
...
def generate_clinical_report(
        ...
        feature_importance,
        crs_df,
        output_file='crs_model_clinical_report.md'):
    ...
    report.append(
        f"This report presents a machine learning model trained specifically on **CRS (Cytokine Release Syndrome) patients** ")
    report.append(
        f"to predict death outcomes. The model uses structured clinical features and achieves:")
    ...
    report.append(
        "The following variables were identified as most important by the model:\n")
    # Categorize features
    ...
    if any('bmi' in f[0].lower() for f in demographic_features):
        report.append(
            "Body mass index (BMI) and weight may be associated with outcomes. ")
    report.append("\n")
    # Clinical features
    ...
    if any('infection' in f[0].lower() for f in clinical_features):
        report.append(
            "Infection-related adverse events are important predictors. ")
    ...
    if any('steroid' in f[0].lower() for f in drug_features):
        report.append(
            "Steroid use (and combinations with antibiotics) is associated with outcomes. ")
    if any('num_drugs' in f[0].lower(
    ) or 'polypharmacy' in f[0].lower() for f in drug_features):
        report.append(
            "The number of concurrent medications (polypharmacy) is important. ")
    report.append("\n")
    # Other features
    if other_features:
    ...
    report.append("\n## Model Performance\n\n")
    report.append(
        f"The {best_model_name} model achieved the following performance:\n\n")
    report.append(
        f"- **ROC-AUC:** {results['roc_auc']:.3f}{Area under the receiver operating characteristic curve)\n")
    report.append(
        f"- **PR-AUC:** {results['pr_auc']:.3f}{Area under the precision-recall curve, important for imbalanced data)\n")
    report.append(
        f"- **F1-Score:** {results['f1']:.3f}{Balanced measure of precision and recall)\n")
...
def run_crs_mortality_pipeline(drug_name="epcoritamab",
    ...
    print("=" * 80)
    print(
        f"CRS Mortality Pipeline: {drug_name} → ae_keyword_list[0] if ae_keyword_list else 'Adverse Event'}")
    print("=" * 80)
    print()
    # Load data
    if not Path(input_csv).exists():
        print(f"ERROR: Data file not found: {input_csv}")
    ...
    case_df = identify_crs_patients(
        df, drug_name=drug_name, ae_keywords=ae_keyword_list)
    if case_df is None or len(case_df) == 0:
        print(
            f"\nERROR: No {drug_name} patients with target AE found. Analysis cannot proceed.")
    return None
    ...
    output_file = Path(output_dir) / \
        f"{drug_name.lower()}_model_clinical_report.md"
...
def main():
    ...
    if crs_df is None or len(crs_df) == 0:
        print("\nERROR: No CRS patients found. Analysis cannot proceed.")
    return
    # Step 2: Build features
    ...
    print(" Skipping SHAP value generation...")
        ...
        print(f"WARNING: SHAP value generation failed: {str(e)[:100]}")
