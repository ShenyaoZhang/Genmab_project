...
if "pr_auc" not in model_results.columns:
    print("ERROR: Error: model_comparison.csv missing pr_auc column; cannot pick best model")
    sys.exit(1)
    best_model_name = model_results["pr_auc"].idxmax()
    best_row = model_results.loc[best_model_name]
    best_model_file = f"trained_model_ best_model_name.lower().replace(
            ' ', '_')}.pkl"

    if not os.path.exists(best_model_file):
        print(f"ERROR: Error: best model file not found {best_model_file}")
...
try:
    ...
    if not HAS_SHAP and not HAS_LIME:
        print("ERROR: Error: both SHAP and LIME unavailable; cannot run explainability")
...
if len(X_test) > MAX_SAMPLES_SHAP:
    sample_idx = np_random.choice(
        len(X_test),
        size=MAX_SAMPLES_SHAP,
        replace=False)
    else:
...
print(
    f" SHAP analysis will use len(X_test_sample)} test samples (random sample, seed={RANDOM_STATE})\n")
...
if HAS_SHAP:
    ...
    print("INFO: Non-tree model or incompatible with TreeExplainer; falling back to shap.Explainer")
    except Exception as e:
        print(f"ERROR: Failed to initialise SHAP explainer: {str(e)[:120]}")
        HAS_SHAP = False
        ...
        if HAS_SHAP and explainer is not None:
            try:
                ...
                # Compute SHAP values
    shap_values = explainer.shap_values(X_test_sample) if hasattr(
        explainer, "shap_values") else explainer(X_test_sample).values
    ...
    shap_outputs.append("shap_summary_plot.png")
        ...
        print(f"WARNING: Failed to generate SHAP summary plot: {str(e)[:120]}")
    ...
    shap_outputs.append("shap_bar_plot.png")
        ...
        print(f"WARNING: Failed to generate SHAP bar plot: {str(e)[:120]}")

    # Individual explanations: plot waterfall for one death/survival case
        try:
            # Calculate expected_value (TreeExplainer typically scalar or
            # length 2)
    try:
    ...
    if isinstance(expected_value, (list, np.ndarray)):
        ...
        expected_value = expected_value[1] if len(
            expected_value) >= 2 else expected_value[0]
        except Exception:
            expected_value = None
    ...
    death_pos = int(
        np.where(
            y_test_sample.values == 1)[0][0]) if (
        y_test_sample == 1).any() else None
    survival_pos = int(
        np.where(
            y_test_sample.values == 0)[0][0]) if (
        y_test_sample == 0).any() else None
    ...
    plt.title(
        f"SHAP Waterfall - {tag.title()} Case (Sample {pos})", fontsize=12, fontweight='bold')
    plt.tight_layout()
    ...
    save_waterfall(survival_pos, "survival")
        ...
        print(
            f"WARNING: Failed to generate individual SHAP waterfall plot: str(e)[
                    :120]}")
    ...
    # Save SHAP values and global importance
        try:
            shap_df = pd.DataFrame(
                shap_matrix, columns=[
                    f"shap_{c}" for c in X_test_sample.columns])

    shap_df["y_true"] = y_test_sample.values
    shap_df.to_csv("shap_values.csv", index=False)
    print(" Saved: shap_values.csv")
    except Exception as e:
        print(f"WARNING: Failed to save shap_values.csv: {str(e)[:120]}")
    ...
    print(" Saved: shap_feature_importance.csv")
        ...
        print(
            f"WARNING: Failed to save shap_feature_importance.csv: str(e)[
                    :120]}")

        print("\n SHAP analysis complete\n")

        except Exception as e:
            print(f"ERROR: SHAP analysis failed: {str(e)[:160]}\n")
...
if HAS_LIME:
    ...
    # LIME needs a predict_proba interface, if not, wrap decision_function
    # with normalisation
    def lime_predict_proba(Xarr):
        Xdf = pd.DataFrame(
            Xarr, columns=X_train.columns) if Xarr.ndim == 2 else pd.DataFrame(
            [Xarr], columns=X_train.columns)
    ...
    death_pos = int(
        np.where(
            y_test.values == 1)[0][0]) if (
        y_test == 1).any() else None
    survival_pos = int(
        np.where(
            y_test.values == 0)[0][0]) if (
        y_test == 0).any() else None
    cases = []
    if death_pos is not None:
        cases.append(("death", death_pos))
    if survival_pos is not None:
    ...
    for tag, pos in cases:
        ...
        plt.title(
            f"LIME Explanation - {tag.title()} Case (Sample {pos})", fontsize=12, fontweight='bold')
        plt.tight_layout()
    ...
    for feat, weight in explanation.as_list():
        ...
        print("\n LIME analysis complete\n")
            ...
            print(f"ERROR: LIME analysis failed: {str(e)[:160]}\n")
