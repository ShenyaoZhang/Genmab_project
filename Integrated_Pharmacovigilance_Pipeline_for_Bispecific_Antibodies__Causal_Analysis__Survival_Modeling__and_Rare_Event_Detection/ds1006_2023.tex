...
\textbf{Primary data source:} The FAERS database (2004-2024) was accessed via the OpenFDA application programming interface (API). A total of 50,000+ adverse event reports were systematically extracted for 37 oncology drugs spanning multiple therapeutic classes:
...
The pipeline architecture is designed to accommodate future incorporation of biomarker data when such information becomes available from prospective clinical studies or electronic health record (EHR) integration:
...
\textbf{Implementation framework:} Upon availability of biomarker data from prospective studies, these variables would be integrated into the feature matrix following standardized preprocessing protocols (log-transformation, normalization, and categorical binning as appropriate for each biomarker's distribution characteristics).
