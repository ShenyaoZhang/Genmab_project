...
\textbf{Primary source:} FAERS database (2004-2024) accessed via OpenFDA API. We systematically extracted 50,000+ adverse event reports for 37 oncology drugs across multiple therapeutic classes:
...
\textbf{Data collection parameters:} 500 records per drug via paginated API requests (limit=100, skip increments), rate-limited to 0.3s between requests to comply with FDA guidelines.

\textbf{Secondary sources:} Instructions and simulated data for EudraVigilance (European) and JADER (Japanese) databases to demonstrate multi-source integration capability for cross-population validation.

\textbf{Drug labels:} FDA drug labels (Section 6: Adverse Reactions) fetched via OpenFDA label endpoint to establish ground truth for known versus unexpected adverse events. MedDRA synonym matching implemented for robust AE term alignment.
...
The Isolation Forest pipeline identified \textbf{185 rare, unexpected drug-event relationships} across 37 oncology drugs after processing 10,000+ initial drug-event pairs.
...
\textbf{1. Integrated multi-method framework:} Unlike single-approach studies, our pipeline combines causal inference, survival analysis, machine learning, and anomaly detection, providing complementary perspectives on drug safety. Each module addresses distinct but related questions:
\begin{itemize}
...
\textbf{CRS incidence rates:}
\begin{itemize}
    \item \textbf{EPCORE NHL-1 trial~\cite{thieblemont2023}:} 49.6\% CRS rate (any grade), 2.5\% Grade $\geq$3
    \item \textbf{Our FAERS data:} 34.4\% CRS rate (343/1000 reports)
    \item \textbf{Interpretation:} Underreporting in FAERS is expected due to voluntary reporting, lack of denominator (total exposed patients), and focus on serious outcomes. Our rate is consistent with post-marketing surveillance patterns.
\end{itemize}
