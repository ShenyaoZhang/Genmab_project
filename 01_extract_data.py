        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if new_records_count == 0 and len(results) > 0:
                    print("   ⚠️  Current page contains only duplicates, stopping collection")
                    break
                ...
                print(f"   ⚠️  HTTP {response.status_code}, retry {retries}/{max_retries} after {delay}s")
                time.sleep(delay)
            ...
            print(f"   ⚠️  Error: {str(e)[:50]}, retry {retries}/{max_retries} after {delay}s")
            time.sleep(delay)
...
def process_record(record, target_drug):
    ...
    try:
        ...
        seriousness_congenital = norm01(record.get('seriousnesscongenitalanomali', 0))
...
def main():
        ...
        try:
            ...
            if records:
                ...
                if before_dedup > after_dedup:
                    print(f"Global dedup: {before_dedup} -> {after_dedup} (-{before_dedup-after_dedup})")
            ...
            print(f"   Total unique reports: {len(all_data)} | Successful drugs: {success_count}")
    ...
    print(f"  Death cases: {int(death_count):,} ({death_count/len(df)*100:.1f}%)")
    print(f"  Hospitalizations: {int(hosp_count):,} ({hosp_count/len(df)*100:.1f}%)")
    print(f"  Life-threatening: {int(life_count):,} ({life_count/len(df)*100:.1f}%)")
    print(f"  Disability: {int(disable_count):,} ({disable_count/len(df)*100:.1f}%)")
    print()
    ...
    if failed_drugs:
        print(f"⚠️  Failed drugs ({len(failed_drugs)}):")
    ...
    with open("task5_collection_summary.txt", "w") as f:
        ...
        f.write(f"Success rate: {success_count}/{len(ONCOLOGY_DRUGS)} ({success_count/len(ONCOLOGY_DRUGS)*100:.1f}%)\n\n")
