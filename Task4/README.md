...
- Fully parameterized: any drug, any AE, no code changes.
- Generates model, SHAP values, summary JSON into `results/{drug}_{ae}/`.
- Includes signal check, database summary, and polypharmacy table helpers.

```bash
# Run full pipeline for any drug/AE
...
# Dataset summary (counts, missingness, CRS prevalence)
python3 scalable_pipeline.py --summary

# Polypharmacy analysis by AE
python3 scalable_pipeline.py --polypharmacy CRS
```

Pipeline flow (slides-ready):
```
...
## Parameterized Pipeline Usage (API)
