...
if "pr_auc" not in model_results.columns:
    print("❌ Error: model_comparison.csv is missing 'pr_auc' column, cannot determine best model")
    sys.exit(1)
...
if not os.path.exists(best_model_file):
    print(f"❌ Error: model file not found {best_model_file}")
    sys.exit(1)
...
if not os.path.exists("X_test.csv") or not os.path.exists("y_test.csv"):
    print("⚠️  Test set not found, falling back to training data for importance analysis (may be optimistic)\n")
    X_imp = X_train.copy()
...
if importances is None:
    print("ℹ️  Falling back to permutation importance (model agnostic)...")
    ...
    try:
        ...
        print(f"❌ Permutation importance failed: {str(e)[:120]}")
        sys.exit(1)
...
plt.title(f"Top {top_n} Feature Importances ({best_model_name} | {use_method})")
