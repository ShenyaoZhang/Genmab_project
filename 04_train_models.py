...
def pick_best_threshold_by_f1(y_true, y_score):
    ...
    return float(thresholds[best_idx]), float(f1_scores[best_idx]), (float(precision[best_idx]), float(recall[best_idx]))
...
for i, (name, model) in enumerate(models.items(), 1):
    ...
    try:
        ...
        if val_scores is None:
            print("⚠️  Model lacks probability/decision function; using default threshold 0.5")
        ...
        print(f"   Validation best threshold (max F1): {best_threshold:.3f} | F1={best_f1_val:.3f}  (P={pr_val:.3f}, R={rc_val:.3f})")
        ...
        print(f"   Test set: PR-AUC={pr_auc:.4f} | ROC-AUC={roc_auc:.4f} | Acc={acc:.4f} | P={prec:.4f} | R={rec:.4f} | F1={f1:.4f}")
        ...
        print(f"   ❌ Training/evaluation failed: {str(e)[:120]}")
        results[name] = {
...
try:
    ...
    if hasattr(best_model, "predict_proba"):
        ...
        with open("trained_model_calibrated.pkl", "wb") as f:
            pickle.dump(calibrator, f)
        print("✅ Saved: trained_model_calibrated.pkl (calibrated)\n")
    else:
        print(f"⚠️  {best_model_name} does not support probability outputs; skipping calibration\n")
except Exception as e:
    print(f"⚠️  Probability calibration failed: {str(e)[:120]}\n")
...
if feature_importance_dict:
    ...
    importance_df.to_csv('feature_importance_complete.csv', index=False)
    print(f"\n💾 Saved: feature_importance_complete.csv ({len(sorted_features)} features)")
    ...
    print("⚠️  Could not extract feature importance from best model")
...
try:
    ...
    shap_df.to_csv('shap_values_complete.csv', index=False)
    print(f"💾 Saved: shap_values_complete.csv (shape: {shap_values.shape})")
    ...
    print(f"\n⚠️  SHAP value generation failed: {str(e)[:100]}")
