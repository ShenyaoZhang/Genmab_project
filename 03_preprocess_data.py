...
if 'safetyreportid' in df.columns:
    ...
    if duplicates_removed > 0:
        print(f"⚠️  Deduplication removed {duplicates_removed} duplicate rows ({duplicates_removed/before_dedup*100:.2f}%)")
...
print("  1️⃣  Convert numeric fields")
print("  2️⃣  Feature engineering (create new features)")
print("  3️⃣  Handle missing values")
print("  4️⃣  Clean data")
print()

# Step 1: convert numeric fields
print("1️⃣  Converting numeric fields...")
...
if leakage_cols:
    raise AssertionError(f"❌ Data leakage! Exclude columns: {leakage_cols}")
...
if min_class_count >= 2:
    ...
    print(f"⚠️  Warning: minimum class size {min_class_count}; using non-stratified split")
    X_train, X_test, y_train, y_test = train_test_split(
        X_imputed, y,
        test_size=0.2,
        random_state=42,
...
if 'safetyreportid' in df.columns:
    ...
    if len(inter) > 0:
        print(f"❌ Warning: ID leakage between train/test! Example IDs: {list(inter)[:5]}")
    else:
