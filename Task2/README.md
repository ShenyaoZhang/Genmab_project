...
```bash
python -c "import lifelines; import sklearn; print('✓ All dependencies installed')"
```
...
```bash
cd task2
python run_survival_analysis.py \
    --drug epcoritamab \
    --adverse_event "cytokine release syndrome" \
    --output_dir results/epcoritamab_crs \
    --limit 1000
```
...
|-----------|-------------------|----------------------|
| **IL-6** | Continuous lab value, log-transform | ↑ IL-6 → ↑ CRS risk (strong) |
| **IL-7** | Continuous lab value, log-transform | ↑ IL-7 → ↑ CRS risk (moderate) |
| **IL-21** | Continuous lab value, log-transform | ↑ IL-21 → ↑ CRS risk (moderate) |
| **Ferritin** | Continuous lab value, log-transform | ↑ Ferritin → ↑ CRS severity (strong) |
| **CRP** | Continuous lab value, log-transform | ↑ CRP → ↑ CRS severity (strong) |
| **IFN-γ** | Continuous lab value, log-transform | ↑ IFN-γ → ↑ CRS risk (strong) |
| **CCL17** | Continuous lab value, log-transform | ↑ CCL17 → ↑ CRS risk |
| **TGF-β1** | Continuous lab value, z-score normalization | ↓ TGF-β1 → ↑ CRS risk (protective) |
